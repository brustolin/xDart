...
extension xString on String {
  ...
  bool isNullOrEmpty({bool checkNullWord = false}) =>
      this == null ||
      this.isEmpty ||
      checkNullWord && this.toLowerCase().trim() == "null";

  ///Check if the string, empty or equals to the "null" ignoring case
  bool isNullOrWhiteSpaces({bool checkNullWord = false}) =>
      this?.trim()?.isNullOrEmpty(checkNullWord: checkNullWord) != false;
}

extension xInt on int {
  ///Transform an int value to its hex value in string
  String toHex([bool withPrefix]) =>
      withPrefix ? "0x${this.toRadixString(16)}" : this.toRadixString(16);
}
...
extension xIterable<T> on Iterable<T> {
  ...
  Map<KEY, List<T>> groupBy<KEY>(KEY getKey(T element)) {
    ...
    this.forEach(
        (item) => res.putIfAbsent(getKey(item), () => List<T>()).add(item));

    return res;
  }

  ///Sum all itens by given rule
  N sum<N extends num>(N getValue(T element)) =>
      this.fold<num>(0, (value, item) => value + getValue(item));
